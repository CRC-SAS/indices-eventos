...
AjustarLMomentosGamma <- function(x, submetodo, min.tasa.valores.positivos) {
  ...
  parametros.ajuste <- list(alpha = NA, beta = NA, prob.0 = NA, prob.media.0 = NA)
  if (length(x) >= (min.tasa.valores.positivos * longitud.inicial)) {
    ...
    if (lmomco::are.lmom.valid(lmom.fit) && !is.na(sum(lmom.fit[[1]])) && !is.nan(sum(lmom.fit[[1]]))) {
      ...
      parametros.ajuste$prob.0       <- prob.cero
      parametros.ajuste$prob.media.0 <- prob.media.mult.ceros  
    } 
  }
...
AjustarNoParametrico <- function(x = NULL, percentiles = NULL, lbound = NULL) {
  ...
  tryCatch({
    ...
    if ((class(objeto.ajuste) != "logspline") || ! is.matrix(objeto.ajuste)) {
      return (NA)
    }
